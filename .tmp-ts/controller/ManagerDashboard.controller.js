...
export default class ManagerDashboard extends Controller {
     ...
     */
    async restoreManagerSession(managerId) {
    ...
    renderUtilizationCharts() {
        ...
        this.byId("currentProjectsUtilized")?.setText(`${currentProjectsUtilized}%`);
        this.byId("currentProjectsAvailable")?.setText(`${currentProjectsAvailable}%`);
        this.byId("caiaUtilized")?.setText(`${caiaUtilized}%`);
        this.byId("caiaAvailable")?.setText(`${caiaAvailable}%`);
        this.byId("pocUtilized")?.setText(`${pocUtilized}%`);
        this.byId("pocAvailable")?.setText(`${pocAvailable}%`);
        ...
        this.renderDonutChart("caiaChart", caiaUtilized, "#f39c12");
        this.renderDonutChart("pocChart", pocUtilized, "#2ecc71");
    }
    /**
    ...
    onApplyVisualizationFilters() {
        const yearSelect = this.byId("yearFilter");
        const quarterSelect = this.byId("quarterFilter");
    ...
    onSearchScopeChange(event) {
        ...
        const managerSelectorBox = this.byId("managerSelectorBox");
    ...
    async onSearchEmployees() {
        const multiInput = this.byId("skillsSearchInput");
        const scopeSelect = this.byId("searchScope");
        const experienceSelect = this.byId("experienceLevel");
        const roleSelect = this.byId("roleFilter");
        const managerSelector = this.byId("managerSelector");
        if (!multiInput) {
    ...
    displaySearchResults(results) {
        const searchResultsPanel = this.byId("searchResultsPanel");
        const searchResultsTable = this.byId("searchResultsTable");
    ...
    async openEmployeeDetailsDialog(employee, isSearchResult) {
        ...
        const dialog = this.byId("employeeDetailsDialog");
        if (!dialog) {
        ...
        try {
            ...
            this.byId("dialogEmployeeName")?.setText(employeeData.name || '');
            this.byId("dialogEmployeeId")?.setText(employeeData.employeeId || '');
            this.byId("dialogEmployeeEmail")?.setText(employeeData.email || '');
            this.byId("dialogEmployeeTeam")?.setText(employeeData.team || '');
            this.byId("dialogEmployeeSpecialization")?.setText(employeeData.specialization || '');
            this.byId("dialogEmployeeManager")?.setText(employeeData.managerId || '');
            // Populate professional details
            this.byId("dialogEmployeeRole")?.setText(profileData.role || 'N/A');
            this.byId("dialogEmployeeLocation")?.setText(profileData.location || 'N/A');
            this.byId("dialogEmployeeTLevel")?.setText(profileData.tLevel || 'N/A');
            this.byId("dialogEmployeeLastUpdated")?.setText(profileData.lastUpdated ? new Date(profileData.lastUpdated).toLocaleDateString() : 'N/A');
            ...
            this.byId("dialogWorkStatus")?.setText(statusText);
            this.byId("dialogWorkStatus")?.setState(statusState);
            ...
            this.byId("dialogActiveProjects")?.setNumber(activeProjects.length);
            this.byId("dialogActiveProjects")?.setUnit(activeProjects.length === 1 ? "project" : "projects");
            // Update skills count
            this.byId("dialogTotalSkills")?.setNumber(skills.length);
            this.byId("dialogTotalSkills")?.setUnit(skills.length === 1 ? "skill" : "skills");
            // Update projects count
            this.byId("dialogTotalProjects")?.setNumber(projects.length);
            this.byId("dialogTotalProjects")?.setUnit(projects.length === 1 ? "project" : "projects");
            // Update certifications count (in Skills tab)
            this.byId("dialogTotalCertifications")?.setNumber(certifications.length);
            this.byId("dialogTotalCertifications")?.setUnit(certifications.length === 1 ? "certification" : "certifications");
            // Update certifications count (in Certifications tab)
            this.byId("dialogTotalCertificationsTab")?.setNumber(certifications.length);
            this.byId("dialogTotalCertificationsTab")?.setUnit(certifications.length === 1 ? "certification" : "certifications");
            // Handle match information for search results
            const matchTab = this.byId("dialogMatchTab");
            if (isSearchResult && employee.matchScore !== undefined) {
                matchTab?.setVisible(true);
                // Set match score progress indicator
                const matchScoreControl = this.byId("dialogMatchScore");
                ...
                const matchLevelControl = this.byId("dialogMatchLevel");
                ...
                this.byId("dialogMatchingSkills")?.setText(employee.matchingSkills || "N/A");
            }
            else {
    ...
    onContactEmployee(event) {
        ...
        const employeeName = this.byId("dialogEmployeeName").getText();
    ...
    openAnalyticsListDialog(title, employees) {
        const dialog = this.byId("analyticsListDialog");
        if (!dialog) {
            MessageToast.show("Dialog not found");
            return;
        }
    ...
    onViewEmployeeDetailsFromAnalytics(event) {
        ...
        const analyticsDialog = this.byId("analyticsListDialog");
        if (analyticsDialog) {
