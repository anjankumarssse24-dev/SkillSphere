...
export default class Landing extends Controller {
    ...
    async onRegisterPress() {
        ...
        const dialog = this.byId("registrationDialog");
        // Reset form to default (Employee)
        const segmentedButton = this.byId("regRoleSegmented");
        segmentedButton.setSelectedKey("Employee");
        this.onRoleChange();
        dialog.open();
    }
    onEmployeeLoginPress() {
    ...
    onSelectManagerRole() {
        ...
        const dialog = this.byId("managerRoleDialog");
        dialog.close();
        this.getRouter().navTo("ManagerLogin");
    }
    onSelectSeniorManagerRole() {
        // Close dialog and navigate to Senior Manager Login
        const dialog = this.byId("managerRoleDialog");
        dialog.close();
        this.getRouter().navTo("SeniorManagerLogin");
    }
    onCloseManagerRoleDialog() {
        const dialog = this.byId("managerRoleDialog");
        dialog.close();
    }
    /**
    ...
    onRoleChange() {
        ...
        const employeeIdSection = this.byId("employeeIdSection");
        const employeeFields = this.byId("employeeSpecificFields");
        const managerFields = this.byId("managerSpecificFields");
        const subTeamSelect = this.byId("regSubTeam");
    ...
    async onRegisterUser() {
        // Get form inputs
        const segmentedButton = this.byId("regRoleSegmented");
        const nameInput = this.byId("regName");
        const emailInput = this.byId("regEmail");
        const experienceInput = this.byId("regExperience");
        const teamSelect = this.byId("regTeam");
        const subTeamSelect = this.byId("regSubTeam");
        const passwordInput = this.byId("regPassword");
        const confirmPasswordInput = this.byId("regConfirmPassword");
        ...
        if (role === "Employee") {
            const employeeIdInput = this.byId("regEmployeeId");
            const managerSelect = this.byId("regManager");
            ...
            const managementAreaInput = this.byId("regManagementArea");
            const managerExperienceInput = this.byId("regManagerExperience");
     ...
     */
    async registerEmployee(employeeId, name, email, team, subTeam, managerId, experience, password) {
     ...
     */
    async registerManager(managerId, name, email, team, subTeam, managementArea, experience, password) {
    ...
    onCloseRegisterDialog() {
        ...
        this.byId("regEmployeeId").setValue("");
        this.byId("regName").setValue("");
        this.byId("regEmail").setValue("");
        this.byId("regExperience").setValue(0);
        this.byId("regTeam").setSelectedKey("");
        this.byId("regSubTeam").setSelectedKey("");
        this.byId("regPassword").setValue("");
        this.byId("regConfirmPassword").setValue("");
        // Employee fields
        const managerSelect = this.byId("regManager");
        if (managerSelect)
            managerSelect.setSelectedKey("");
        // Manager fields
        const managementAreaInput = this.byId("regManagementArea");
        if (managementAreaInput)
            managementAreaInput.setValue("");
        const managerExperienceInput = this.byId("regManagerExperience");
        if (managerExperienceInput)
            managerExperienceInput.setValue(0);
