...
export default class EmployeeDashboard extends Controller {
    ...
    async loadSkills(employeeId) {
        try {
            ...
            const skills = skillsContexts.map((ctx) => ctx.getObject());
    ...
    async loadProjects(employeeId) {
        try {
            ...
            const projects = projectsContexts.map((ctx) => ctx.getObject());
    ...
    async loadCurrentProjects(employeeId) {
        try {
            const oDataModel = this.getOwnerComponent()?.getModel();
    ...
    async loadCAIAUtilization(employeeId) {
        try {
            const oDataModel = this.getOwnerComponent()?.getModel();
    ...
    async loadPOCUtilization(employeeId) {
        try {
            const oDataModel = this.getOwnerComponent()?.getModel();
    ...
    async loadCertifications(employeeId) {
        try {
            const oDataModel = this.getOwnerComponent()?.getModel();
            const binding = oDataModel.bindList("/Certifications");
            binding.filter([new Filter("employeeId", FilterOperator.EQ, employeeId)]);
            const contexts = await binding.requestContexts(0, 100);
            const certifications = contexts.map((ctx) => {
                const obj = ctx.getObject();
                const formatted = {
                    ...obj,
    ...
    // ============ FORMATTERS ============
    formatProficiencyState(proficiency) {
        switch (proficiency) {
            case "Expert":
            case "Advanced":
            case "Proficient":
    ...
    formatCertificationState(certification) {
    ...
    async onSaveSkill() {
        try {
            ...
            const newSkillModel = this.getView()?.getModel("newSkill");
    ...
    onSkillCategoryChange(event) {
        ...
        const skillCatalogModel = this.getView()?.getModel("skillCatalog");
    ...
    onEditSkillCategoryChange(event) {
        const select = event.getSource();
        const selectedCategory = select.getSelectedKey();
        ...
        const editSkillModel = this.getView()?.getModel("editSkill");
        if (editSkillModel) {
    ...
    populateEditSkillCatalog(selectedCategory) {
        ...
        const editSkillCatalogModel = this.getView()?.getModel("editSkillCatalog");
    ...
    onEditCertificationChange(event) {
        const select = event.getSource();
        const selectedKey = select.getSelectedKey();
        const editSkillModel = this.getView()?.getModel("editSkill");
        if (editSkillModel) {
    ...
    async onEditSkill(event) {
        try {
            const context = event.getSource().getBindingContext("skills");
    ...
    async onSaveProject() {
        try {
            ...
            const newProjectModel = this.getView()?.getModel("newProject");
    ...
    onCloseAddProjectDialog() {
        ...
        this.byId("statusSelect")?.setSelectedKey("Active");
        this.byId("descriptionInput")?.setValue("");
    }
    async onEditProject(event) {
        try {
            const context = event.getSource().getBindingContext("projects");
    ...
    async onSaveEditedProject() {
        try {
            const editProjectModel = this.getView()?.getModel("editProject");
    ...
    async onSaveProfile() {
        try {
            ...
            const profileModel = this.getView()?.getModel("profile");
    ...
    async onSaveCurrentProject() {
        const dialogModel = this.getView()?.getModel("currentProjectDialog");
    ...
    onCloseCurrentProjectDialog() {
        this.currentProjectDialog?.close();
    }
    async onEditCurrentProject(event) {
    ...
    async onDeleteCurrentProject(event) {
        const source = event.getSource();
        const bindingContext = source.getBindingContext("currentProjects");
        const currentProject = bindingContext?.getObject();
        try {
    ...
    async onSaveCAIA() {
        const dialogModel = this.getView()?.getModel("caiaDialog");
        const data = dialogModel.getData();
        const employeeId = this.currentEmployeeId;
        // Validation
    ...
    async onSavePOC() {
        const dialogModel = this.getView()?.getModel("pocDialog");
        const data = dialogModel.getData();
        const employeeId = this.currentEmployeeId;
        // Validation
    ...
    async onSaveCertification() {
        const dialogModel = this.getView()?.getModel("certificationDialog");
        const data = dialogModel.getData();
        const employeeId = this.currentEmployeeId;
        // Validation
    ...
    async onEditCertification(event) {
        const source = event.getSource();
    ...
    async onDeleteCertification(event) {
        const source = event.getSource();
        const bindingContext = source.getBindingContext("certifications");
        const certification = bindingContext?.getObject();
        try {
    ...
    currentChatEmployeeId = ""; // Track which employee's chat is currently displayed
    /**
    ...
    onOpenAIAssistant() {
        ...
        const oContainer = this.byId("messagesContainerEmployee");
        if (!oContainer || oContainer.getItems().length === 0) {
            this.initializeAIChat();
        }
        const oDialog = this.byId("aiAssistantDialogEmployee");
        oDialog?.open();
    }
    /**
     * Clear chat for new employee - called when employee ID changes
     */
    clearChatForNewEmployee() {
    ...
    initializeAIChat() {
        ...
        this.addBotMessage(`👋 Hello ${employeeName}! I'm your AI assistant.\n\n` +
            "I can help you with:\n" +
    ...
    onQuickAction(oEvent) {
        ...
        const input = this.byId("messageInputEmployee");
     ...
     */
    onSendMessage() {
        const oInput = this.byId("messageInputEmployee");
    ...
    scrollToBottom() {
        setTimeout(() => {
            const oScrollContainer = this.byId("chatContainerEmployee");
            if (oScrollContainer) {
